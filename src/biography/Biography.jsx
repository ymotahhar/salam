...
import {
  ...
  BioFilmItem,
  SelectedBioItem,
  SelectedBioFilmTitle,
  GenereList,
  Genere, BioFilmDetail, BioTitr, FreeLable, CloseBiofilm
} from "./Biography.styled";
import SelectedBioFilmData from "./biographyfilm/SelectedBioFilmData"
...
export default function BioFilm() {
  const [bioFilmList, setBioFilmList] = useState([]);
  const [selectedBioFilm, setSelectedBioFilm] = useState();
  const [selectedFilmHover, setSelectedFilmHover] = useState();
  const [displayed, setDisplayed] = useState(false);
  const [displayedItem, setDisplayedItem] = useState(false);
  const [isVisibleBioFilm, setIsVisibleBioFilm] = useState(false)
  useEffect(() => {
    axios.get("https://moviesapi.ir/api/v1/genres/4/movies?page=1").then((res) =>
      setBioFilmList(res.data.data));
    // {
    //   "id": 1,
  ...
  function closebiofilm() {
    setIsVisibleBioFilm(false);
  }
  function showbiofilm() {
    setIsVisibleBioFilm(true);
  }
    ...
    <div >
      ...
      >
        <BioFilmList>
          ...
          {bioFilmList.map((film) => {
            return (<SwiperSlide>

              <BioFilmItem className="freelable">
                <div
                  onMouseEnter={() => {
                ...
                >
                  ...
                  <BioFilmTitle>{film.title}</BioFilmTitle>
                </div>
              </BioFilmItem>
            </SwiperSlide>
      ...
      {
        selectedBioFilm && (
          <SelectedBioItem showBioFilm={isVisibleBioFilm} >
            <SelectedBioFilmData idBio={selectedBioFilm.id}></SelectedBioFilmData>
            <CloseBiofilm onClick={closebiofilm}   ></CloseBiofilm>
            <img src={selectedBioFilm.poster}></img>
            {/* <SelectedBioFilmTitle>{selectedBioFilm.title}</SelectedBioFilmTitle>
            <GenereList> genere : {selectedBioFilm.genres.map((gen) => {
