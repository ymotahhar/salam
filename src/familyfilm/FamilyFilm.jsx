...
import {
  ...
  FamilyFilmItem,
  SelectedFamilyItem,
  SelectedFamilyFilmTitle,
  GenereList,
  Genere, FamilyFilmDetail, FamilyTitr, FreeLable, CloseFamilyfilm
} from "./FamilyFilm.styled";
import SelectedFamilyFilmData from "./familyfilmdata/SelectedFamilyFilmData"
...
export default function WarFilm() {
  const [familyFilmList, setFamilyFilmList] = useState([]);
  const [selectedFamilyFilm, setSelectedFamilyFilm] = useState();
  const [selectedFilmHover, setSelectedFilmHover] = useState();
  const [displayed, setDisplayed] = useState(false);
  const [displayedItem, setDisplayedItem] = useState(false);
  const [isVisibleFamilyFilm, setIsVisibleFamilyFilm] = useState(false)
  useEffect(() => {
    axios.get("https://moviesapi.ir/api/v1/genres/13/movies?page=1").then((res) =>
      setFamilyFilmList(res.data.data));
    // {
    //   "id": 1,
  ...
  function closefamilyfilm() {
    setIsVisibleFamilyFilm(false);
  }
  function showfamilyfilm() {
    setIsVisibleFamilyFilm(true);
  }
    ...
    <div >
      ...
      >
        <FamilyFilmList>
          ...
          {familyFilmList.map((film) => {
            return (<SwiperSlide>

              <FamilyFilmItem className="freelable">
                <div
                  onMouseEnter={() => {
                ...
                >
                  ...
                  <FamilyFilmTitle>{film.title}</FamilyFilmTitle>
                </div>
              </FamilyFilmItem>
            </SwiperSlide>
      ...
      {
        selectedFamilyFilm && (
          <SelectedFamilyItem showFamilyFilm={isVisibleFamilyFilm} >
            <SelectedFamilyFilmData idFamily={selectedFamilyFilm.id}></SelectedFamilyFilmData>
            <CloseFamilyfilm onClick={closefamilyfilm}   ></CloseFamilyfilm>
            <img src={selectedFamilyFilm.poster}></img>
            {/* <SelectedFamilyFilmTitle>{selectedFamilyFilm.title}</SelectedFamilyFilmTitle>
            <GenereList> genere : {selectedFamilyFilm.genres.map((gen) => {
