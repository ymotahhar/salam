...
import {
   ...
   GenereList,
   Genere
} from "./SelectedFamilyfilmData.styled";
import { useState, useEffect } from "react";
export default function SelectedFamilyFilmData(props) {
   const [dataSelectedFamilyFilm, setDataSelectedFamilyFilm] = useState([]);
   useEffect(() => {
      axios.get("https://moviesapi.ir/api/v1/movies/" + props.idFamily).then((res) =>
         setDataSelectedFamilyFilm(res.data));
   }, [props.idFamily]);
   return (
      <>
         <AboutSelectedFamilyFilm>
            <SelectedFamilyFilmTitle>{dataSelectedFamilyFilm.title}</SelectedFamilyFilmTitle>
            <FQuality>
               <FRunTime>{dataSelectedFamilyFilm.runtime}</FRunTime>
               <FYear>{dataSelectedFamilyFilm.year}</FYear>
               <FRate><span>
             ...
             </span>{dataSelectedFamilyFilm.imdb_rating}</FRate>
               <FCountry>{dataSelectedFamilyFilm.country}</FCountry>
            </FQuality>
            <FPlot>{dataSelectedFamilyFilm.plot}</FPlot>
            <FPlay>ورود و پخش
            ...
            <FPlayer><span>Actors : </span>{dataSelectedFamilyFilm.actors}</FPlayer>
            <FDirector><span>Director : </span>{dataSelectedFamilyFilm.director}</FDirector>
