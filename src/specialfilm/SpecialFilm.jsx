...
import {
  ...
  FilmItem,
  SelectedItem,
  SpecialTitr, FilmDetail, CloseSpecialfilm
} from "./specialfilm.styled";
import { Swiper, SwiperSlide } from "swiper/react";
import "swiper/css";
import SelectedFilmData from "./aboutselectedfilm/SelectedFilmData";
import 'swiper/css/free-mode';
import { Pagination, Autoplay, FreeMode, Navigation, Thumbs } from 'swiper/modules';
import 'swiper/css/thumbs';
import { colors } from "@mui/material";
// import "./slider.css"
export default function SpecialFilm() {
  const [specialFilmList, setSpecialFilmList] = useState([]);
  const [selectedSpecialFilm, setSelectedSpecialFilm] = useState();
  const [selectedFilmHover, setSelectedFilmHover] = useState();
  const [displayed, setDisplayed] = useState(false);
  const [displayedItem, setDisplayedItem] = useState(false);
 
  useEffect(() => {
    axios.get("https://moviesapi.ir/api/v1/movies?page=13").then((res) =>
      setSpecialFilmList(res.data.data));
  }, []);
  const [isVisible, setIsVisible] = useState(false)
  function closespecialfilmitem() {
    setIsVisible(false)
     }
  function showspecialfilmitem() {
    setIsVisible(true)
  ...
  return (
    < div >
      ...
      {
        selectedSpecialFilm && (
          <SelectedItem show={isVisible}>
            <SelectedFilmData id={selectedSpecialFilm.id}></SelectedFilmData>
            <CloseSpecialfilm onClick={closespecialfilmitem}  ></CloseSpecialfilm>
            <img src={selectedSpecialFilm.poster}></img>
          </SelectedItem>
