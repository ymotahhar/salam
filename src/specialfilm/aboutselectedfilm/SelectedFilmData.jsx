...
import {
   AboutSelectedSpecialFilm,
   SelectedSpecialFilmTitle,
   Quality,
   RunTime,
   Year,
   Rate,
   ...
   GenereList,
   Genere
} from "./SelectedFilmData.styled";
import { useState, useEffect } from "react";
export default function SelectedFilmData(props) {
   const [dataSelectedFilm, setDataSelectedFilm] = useState([]);
   useEffect(() => {
      axios.get("https://moviesapi.ir/api/v1/movies/" + props.id).then((res) =>
         setDataSelectedFilm(res.data));
   }, [props.id]);
   return (
      <>
         <AboutSelectedSpecialFilm>
            <SelectedSpecialFilmTitle>{dataSelectedFilm.title}</SelectedSpecialFilmTitle>
            <Quality>
               <RunTime>{dataSelectedFilm.runtime}</RunTime>
               <Year>{dataSelectedFilm.year}</Year>
               <Rate><span>
              ...
              </span>{ dataSelectedFilm.imdb_rating}</Rate>
               <Country>{dataSelectedFilm.country}</Country>
            </Quality>
            <Plot>{dataSelectedFilm.plot}</Plot>
            <Play>ورود و پخش
            ...
            <Player><span>Actors : </span>{dataSelectedFilm.actors}</Player>
            <Director><span>Director : </span>{dataSelectedFilm.director}</Director>
